...
> \`leave\` , \`Prefix: -\`
>     Made by Shahzain`)
            .setFooter("©️ 2020 SteeZ eSports™|Shahzain", "https://cdn.discordapp.com/attachments/732200726461284352/755493585893326908/Clan_logo.png");
    ...
    if (command === "search" || command === "sc") {
        const voiceChannel = message.member.voice.channel;
        if (!voiceChannel) return message.channel.send({embed: {color: "RED", description: "I'm sorry, but you need to be in a voice channel to play a song!"}});
        const permissions = voiceChannel.permissionsFor(message.client.user);
        if (!permissions.has("CONNECT")) {
            return message.channel.send({embed: {color: "RED", description: "Sorry, but I need a **`CONNECT`** permission to proceed!"}});
        }
    ...
    } else if (command === "stop" || command === "st" || command === "leave") {
